...
public class Main {
	...
	public static void solution(int[] people, int nskills, int cp, ArrayList<Integer> onesol, int skills) {
	    ...
	    if(cp == people.length) // if arraylist completed
	    {
	        if(skills == ((1<<nskills)-1)) // if all skills are available
	        {
		...
		solution(people,nskills,cp+1,onesol,skills); // if no option in recursion.
		onesol.add(cp); // then add in onesol.
